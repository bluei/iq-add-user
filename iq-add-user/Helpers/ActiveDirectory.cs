...
namespace iq_add_user.Helpers
{
    public class ActiveDirectory
    {
        #region Variables

        private static string sDomain = WebConfigurationManager.AppSettings["AdServer"];
        private static string sExpenseOU = WebConfigurationManager.AppSettings["AdExpenseOU"];
        private static string sDefaultRootOU = WebConfigurationManager.AppSettings["AdRootOU"];
        private static string sServiceUser = WebConfigurationManager.AppSettings["AdAdminUsr"];
        private static string sServicePassword = WebConfigurationManager.AppSettings["AdAdminPwd"];
        ...
        {
            PrincipalContext oPrincipalContext = new PrincipalContext(ContextType.Domain, sDomain, sDefaultRootOU, ContextOptions.SimpleBind, sServiceUser, sServicePassword);
        ...
        /// <summary>
        /// Used in the Employee controller to add an expense group,
        /// with members including the AP group, the User AD account, and the Approver's account.
        /// </summary>
        /// <param name="sGroupName"></param>
        /// <returns>Returns the message result that will be displayed back in the view result.</returns>
        public static string AddExpenseGroup(string sGroupName, string userSAM, string approverSAM)
        {
            string result; ;
            try
            {
                // Get the Root OU
                PrincipalContext oRootContext = new PrincipalContext(
                    ContextType.Domain,
                    sDomain, sDefaultRootOU,
                    ContextOptions.SimpleBind,
                    sServiceUser,
                    sServicePassword);

                // Get the Expense Report OU
                PrincipalContext oExpenseReportContext = new PrincipalContext(
                    ContextType.Domain,
                    sDomain, sExpenseOU,
                    ContextOptions.SimpleBind,
                    sServiceUser,
                ...
                if (GetGroup(sGroupName) == null)
                {
                    // Group does not exist so create it.
                    GroupPrincipal oGroupPrincipal = new GroupPrincipal(oExpenseReportContext, sGroupName);
                    oGroupPrincipal.GroupScope = GroupScope.Global;
                    oGroupPrincipal.IsSecurityGroup = true;
                    oGroupPrincipal.Save();

                    result = string.Format("Active Directory Group [{0}] was added", sGroupName);


                    // ADD MEMBERS
                    // GroupPrincipal oExpenseAPGroup = GroupPrincipal.FindByIdentity(oRootContext, "Expense AP");
                    UserPrincipal oExpenseApproverUser = UserPrincipal.FindByIdentity(oRootContext, IdentityType.SamAccountName, approverSAM);
                    UserPrincipal oCurrentUser = UserPrincipal.FindByIdentity(oRootContext, IdentityType.SamAccountName, userSAM);

                    //oGroupPrincipal = GroupPrincipal.FindByIdentity(oExpenseReportContext, sGroupName);

                    // add expense approver account
                    if (oGroupPrincipal != null && oExpenseApproverUser != null)
                    {
                        oGroupPrincipal.Members.Add(oExpenseApproverUser);
                        oGroupPrincipal.Save();
                        result += string.Format(" including expense approver {0}", approverSAM);
                    }
                    else
                    {
                        result += string.Format(" EXCEPT expense approver {0} (not found in AD)", approverSAM);
                    }

                    // add user account
                    if (oGroupPrincipal != null && oCurrentUser != null)
                    {
                        oGroupPrincipal.Members.Add(oCurrentUser);
                        oGroupPrincipal.Save();
                        result += string.Format(", along with user {0}", userSAM);
                    }
                    else
                    {
                        result += string.Format(", EXCEPT user {0} (not found in AD)", userSAM);
                    }
                }
                else
                {
                    result = string.Format("Active Directory Group [{0}] already existed.", sGroupName);
                }

            }
            catch (Exception e)
            {
                return string.Format("ERROR adding expense group:{0}{1}", Environment.NewLine, e.Message);
            }
            return result;
        }

        public static string GrantModifyOnPathToGroup(string directoryPath, string groupToAdd)
        {
            if (GetGroup(groupToAdd) == null)
            {
                return string.Format("* Group [{0}] was not found. Permissions on folder [{1}] could not be set.", groupToAdd, directoryPath);
            }
            else
            {
                try
                {
                    ...
                    return string.Format("Granted [Modify] permissions to {0} on folder {1}", groupToAdd, directoryPath);
                }
                catch (Exception e)
                {
                    return string.Format("Folder permissions were NOT added.  Something went wrong:{0}{1}", Environment.NewLine, e.Message);
                }
            }
        }
